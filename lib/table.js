...
Table.prototype.allPlayersTalked = function() {
	...
	while (endOfRound && i < this.players.length) {
		...
		if(!this.players[i].isEmptySeat && !this.players[i].talked && !this.players[i].folded && !this.players[i].isAllIn) {
	...
	this.forEachPlayers(function(player, $index) {
		...
		if(!player.folded) {
			if(playerRank === maxRank && maxRank > 0) {
				winners.push($index);
				console.log('player ' + player.playerName + ' ties with ' + player.hand.message + '(cards: ' + player.hand.cards + ', value ' + player.hand.rank + ')');
			} else if(playerRank > maxRank) {
				maxRank = playerRank;
				winners = [$index];
				console.log('player ' + player.playerName + ' takes the lead with ' + player.hand.message + '(cards: ' + player.hand.cards + ', value ' + player.hand.rank + ')');
			} else {
				console.log('player ' + player.playerName + ' shows a pathetic ' + player.hand.message + '(cards: ' + player.hand.cards + ', value ' + player.hand.rank + ')');
			} 
		}
	});
...
Table.prototype.checkForWinner = function() {
	...
	while (!this.roundEnd()) {
		...
		if(allInWinners.length > 0) {
			...
			part = parseInt(this.getMinBets(allInWinners, winners), 10);
...
Table.prototype.checkForBankrupt = function() {
	for (var i = this.players.length - 1; i >= 0; i--) {
		...
		if(this.players[i].chips <= 0) {
			...
			this.emit('bankrupt', this.players[i]);
...
Table.prototype.getCurrentPlayerLabel = function(index) {
	var player = index ? this.players[index] : this.getCurrentPlayer();
...
Table.prototype.NewRound = function() {
	...
	if(this.getIngamePlayersLength() > 2) {
		console.log(this.getCurrentPlayerLabel(this.dealer), 'deals');
	...
	this.players[smallBlind].SimpleBet(this.smallBlind, true);
	console.log(this.getCurrentPlayerLabel(smallBlind), 'pays small blind', this.smallBlind);
	if(!this.players[smallBlind].isAllIn) {
		this.players[smallBlind].talked = false;
	}

	this.currentPlayer = bigBlind;
	this.players[bigBlind].SimpleBet(this.bigBlind, true);
	console.log(this.getCurrentPlayerLabel(bigBlind), 'pays big blind', this.bigBlind);
...
Table.prototype.NextPlayer = function() {
	...
	var callAmount = cp.callAmount();
	var turnMessage = 'check, bet, fold';
	if(callAmount > 0) {
		turnMessage = 'call ' + callAmount + ', raise, fold';
	}
	console.log(this.getCurrentPlayerLabel(self.currentPlayer) + ' next turn', turnMessage);
