...
Table.prototype.moveBetsToPot = function() {
    for (var i = 0; i < this.game.bets.length; i++) {
        this.game.pot = this.game.pot + parseInt(this.game.bets[i], 10);
        this.game.roundBets[i] = this.game.roundBets[i] + parseInt(this.game.bets[i], 10);
        this.game.bets[i] = 0;
    }
};
...
Table.prototype.setCurrentPlayerToSmallBlind = function() {
    this.currentPlayer = this.getNextPlayerIndex(this.dealer);
};
...
Table.prototype.progress = function() {
    ...
    console.log('this.allPlayersTalked()', this.allPlayersTalked());
...
Table.prototype.NewRound = function() {
    ...
    if(!this.players[bigBlind].allIn) {
...
Table.prototype.resetTalkedStatusOnRaise = function() {
    this.forEachPlayers(function(player) {
        if(!player.folded && !player.allIn && player.GetBet() < this.getMaxBet()) {
            player.talked = false;
        }
    });
};
