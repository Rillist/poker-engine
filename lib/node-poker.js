...
function Table(smallBlind, bigBlind, minPlayers, maxPlayers, minBuyIn, maxBuyIn, autoStart) {
...
function checkForEndOfRound(table) {
    ...
    var playerNextToCurrent = table.currentPlayer + 1 >= table.players.length ? 0 : table.currentPlayer + 1;
    var currentPlayerHasMoved;
    ...
    var checkPlayer = function(player) {
        if(!player.hasPlayed()) {
            ...
            if(!currentPlayerHasMoved) {
                ...
                table.currentPlayer = player.getIndex();
            }
        }
    };
    ...
    while(i < len && endOfRound) {

        var current = i + playerNextToCurrent;
        current = current >= len ? current - len : current;

        checkPlayer(table.players[current]);
...
function checkForWinner(table) {
    ...
    if(allInPlayer.length > 0) {
        ...
        part = parseInt(minBets, 10) || 0;
    } else {
        part = parseInt(table.game.roundBets[winners[0]], 10) || 0;
    ...
    if(table.game) {
        ...
        if(checkForEndOfRound(table) === true) {
            ...
            for (i = 0; i < table.game.bets.length; i += 1) {
                table.game.pot = table.game.pot + (parseInt(table.game.bets[i], 10) || 0);
                table.game.roundBets[i] = table.game.roundBets[i] + (parseInt(table.game.bets[i], 10) || 0);
            }
            ...
            table.currentPlayer = table.currentPlayer >= table.players.length - 1 ? 0 : table.currentPlayer + 1;
        }
    }
}
...
Table.prototype.check = function(playerName) {
    ...
    if(this.players[ currentPlayer ] && playerName === this.players[ currentPlayer ].playerName) {
...
var isCurrentPlayer = function(table, playerName) {
    var currentPlayer = table.currentPlayer;
    return table.players[ currentPlayer ] && playerName === table.players[ currentPlayer ].playerName;
};

Table.prototype.fold = function(playerName) {
    if(isCurrentPlayer(this, playerName)) {
...
Table.prototype.call = function(playerName) {
    ...
    if(isCurrentPlayer(this, playerName)) {
        ...
        this.players[ this.currentPlayer ].Call();
        return true;
...
Table.prototype.bet = function(playerName, amount) {
...
Table.prototype.AddPlayer = function(playerName, chips) {
    ...
    if(this.autoStart && this.players.length === 0 && this.playersToAdd.length >= this.minPlayers) {
        this.StartGame();
    }
};
Table.prototype.removePlayer = function(playerName) {
    ...
    for (var j = 0; j < this.playersToAdd.length; j++) {
        if(this.playersToAdd[j].playerName === playerName) {
            this.playersToAdd.splice(j, 1);
        }
    }
};
Table.prototype.NewRound = function() {
    ...
    var j, smallBlind, bigBlind;
    //Deal 2 cards to each player
    for (j = 0; j < this.players.length; j++) {
        this.players[j].cards.push(this.game.deck.pop());
        this.players[j].cards.push(this.game.deck.pop());
        this.game.bets[j] = 0;
        this.game.roundBets[j] = 0;
    }
    ...
    //Force Blind Bets
    var forceBlindBet = function(table, playerId, blind) {
        if(table.players[playerId].chips > blind) {
            table.game.bets[playerId] = blind;
            table.players[playerId].chips = table.players[playerId].chips - blind;
        } else {
            // is allin
            table.game.bets[playerId] = table.players[playerId].chips;
            table.players[playerId].chips = 0;

            // force state
            table.players[playerId].allIn = true;
            table.players[playerId].talked = true;
        }
    };

    forceBlindBet(this, smallBlind, this.smallBlind);
    forceBlindBet(this, bigBlind, this.bigBlind);
...
Player.prototype.hasPlayed = function() {
    ...
    if((player.folded === false) &&
        (player.talked === false || table.game.bets[i] !== maxBet) &&
        (player.allIn === false)) {
...
var getPlayerIndex = function(player) {
    var index = -1, i;
    for (i = 0; i < player.table.players.length; i += 1) {
        if(player === player.table.players[i]) {
            index = i;
        }
    }
    return index;
...
Player.prototype.Check = function() {
    var checkAllow, v, playerIndex;
    checkAllow = true;
    playerIndex = getPlayerIndex(this);
    var playersBet = this.table.game.bets[playerIndex];

    for (v = 0; v < this.table.game.bets.length; v += 1) {
        if(this.table.game.bets[v] > playersBet) {
            checkAllow = false;
        }
    }
    if(checkAllow) {
        //this.table.game.bets[playerIndex] = playersBet;
        this.talked = true;
...
Player.prototype.Fold = function() {
    ...
    for (i = 0; i < this.table.players.length; i += 1) {
        if(this === this.table.players[i]) {
            bet = parseInt(this.table.game.bets[i], 10) || 0;
            this.table.game.bets[i] = 0;
            this.table.game.pot = this.table.game.pot + bet;
            this.talked = true;
        }
    }
...
var getNew = function(params, players) {
    ...
    // create game
    // Table params : smallBlind, bigBlind, minPlayers, maxPlayers, minBuyIn, maxBuyIn
    // add players
    ...
    players.forEach(function(player) {
        ...
        table.AddPlayer(playerName, params.defaultStack);
    });
...
var load = function(data) {
    ...
    // create new game
    var table = new Table(smallBlind, bigBlind, minPlayers, maxPlayers, minBuyIn, maxBuyIn);

    var restorePlayer = function(playersData, table) {
        var player = new Player(playersData.playerName, playersData.chips, table);
