...
Table.prototype.toJSON = function() {
    ...
    var clean = function(obj, space) {
        obj.forEach(function(item) {
            if(typeof(item[space]) !== 'undefined') {
                delete item[space];
            }
        });
        return obj;
    ...
    if(data.players && data.players.length) {
        data.players = clean(data.players, objToRemove);
    }

    if(data.playersToAdd && data.playersToAdd.length) {
        data.playersToAdd = clean(data.playersToAdd, objToRemove);
    }

    if(data.playersToRemove && data.playersToRemove.length) {
        data.playersToRemove = clean(data.playersToRemove, objToRemove);
    }

    if(data.gameLosers && data.gameLosers.length) {
        data.gameLosers = clean(data.gameLosers, objToRemove);
    }
