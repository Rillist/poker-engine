...
var hasPlayed = function(table, i) {
    var played = true,
...
function checkForEndOfRound(table) {
    ...
    var checkPlayer = function(id) {
        if(!hasPlayed(table, id)) {
            endOfRound = false;
    ...
    for (i = playerNextToCurrent; i < table.players.length; i++) {
        checkPlayer(i);
    }

    for (j = 0; j < playerNextToCurrent; j++) {
        checkPlayer(j);
    }
